            ...
            <div class="row">
                <div class="col-md-9 col-sm-12">
                    <div class="single_post">
                        ...
                        <div class="post_title">
                            <div class="post_container">
                                ...
                                @if ($post['userId'] == Auth::id() || Auth::user()?->role == 'admin')
                                    <div class="btn_group"
                                        ...
                                        <a href="{{ route('toupdate', $post['id']) }}" class="btn green"
                                            style="margin-right: 5px">Редактировать</a>
                            ...
                            <div style="display: flex; align-items-center; justify-content: start">
                                <div><span class="post_date">Date : {{ Str::limit($post['created_at'], 10, '') }}</span>
                                </div>
                    ...
                    <div class="pages_pagination">
                        @if ($prevPage)
                            <a href="{{ route('single', $prevPage) }}" class="prev"><i
                                    class="icon ion-arrow-left-c"></i></a>
                        @endif
                        @if ($nextPage)
                            <a href="{{ route('single', $nextPage) }}" class="next"><i
                                ...
                                <div class="tab_single shown">
                                    @foreach ($popularPosts as $item)
                                        <div class="related_post">
                                            @if ($item['image'])
                                                <div class="thumb"><img src="{{ asset('storage/' . $item['image']) }}"
                                                        alt="image" style="width: 60px; height: 60px"></div>
                                            @else
                                                <div class="thumb"><img src="{{ asset('img/thumb.jpg') }}" alt="image">
                                                </div>
                                            @endif
                                            <a href="{{ route('single', $item['id']) }}"
                                                class="post_title montserrat-text uppercase">{{ $item['name'] }}</a>
                                            <div class="post_date">{{ $item['created_at']->day }}
                                                {{ $item['created_at']->format('F') }} {{ $item['created_at']->year }}
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                                    ...
                                    <li class="testimonials_single" style="margin-bottom: 30px">
                                        <a href="{{ route('profile', $item['userId']) }}">
                                            <div class="author_pic"
                                                style="width: 75px; height: 75px; margin-bottom: 20px">
                                                @if ($item->getUser()->avatar)
                                                    <img src="{{ asset('storage/' . $item->getUser()->avatar) }}"
                                                        alt="avatar" style="width: 75px; height: 75px">
                                                @else
                                                    <img src="{{ asset('img/face.jpg') }}" alt="avatar"
                                                        style="width: 75px; height: 75px">
                                                @endif
                                            </div>
                                            <div class="author_name" style="margin-bottom: 10px">
                                                {{ $item->getUser()->name }}</div>
                                        </a>
                                        ...
                                        @if (Auth::id() == $item['userId'] || Auth::user()?->role == 'admin')
                                            <div style="display: flex; justify-content: flex-end">
                                                ...
                                                <form action="{{ route('toupdatecomment', $item['id']) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <input type="text" name="comment" id="comment"
                                                        class="input_comment" style="display: none">
                                                    <button type="submit"
                                                        class="btn green comment_btn_ready">Готово</button>
                                                </form>
                                                <form action="{{ route('todeletecomment', $item['id']) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="col-md-12">
                    <div class="row">
                        <div class="white-section" style="padding:20px">
                            <span class="montserrat-text uppercase" style="font-size:24px">let's express our
                                opinion</span>
                            @error('comment')
                            ...
                            <form action="{{ route('tocomment') }}" method="POST">
                                @csrf
                                <input class="invisible" type="text" name="postId" id="postId"
                                    value="{{ $post['id'] }}">
                                ...
                                <button type="submit" class="btn green"
                                    style="margin-top:20px"><span>comment</span></button>
                            </form>
                        </div>
                    </div>
                </div>
